...
export const formColumnsByLessons = (lessons: ILessonWithMarks[]) => {
  ...
  export const isLessonInThisDate = (date: number, daysWithLessons: DaysWithLessons) => {
    return daysWithLessons.find((e) => e.date === date) ? true : false;
  };
  export const getAttributtes = (lessonDate: number, daysWithLessons: DaysWithLessons) => {
    const lesson = daysWithLessons.find((e) => e.date === lessonDate);
    if (!lesson) return;
    return {lessonId: lesson.lessonId, teacherId: lesson.teacherId}
  }

export const findMarkOfLessonDateAndStudentId = (lessonDate, studentID) => {
  const lesson = daysWithLessons.find((e) => e.date === lessonDate);
  if (!lesson) return;
